(function($, _, chrome, document, window){
	...
	chrome.browserAction.onClicked.addListener(function (tab) {
		linkAudit.log('attempting check');
		chrome.tabs.executeScript(tab.id, {
            code:'if(!window.linkAudit){alert("Link Audit has just been installed and this page needs to be reloaded before Link Audit can run")}'
        });
	});
    ...
    var DEFAULTS = window.DEFAULTS,
        ...
        validateLinks:function(){
            ...
            for (var i = 0, l = linkAudit.linkArray.length, max = parseInt(linkAudit.getLocalStore("maxQueueLength"),10); (i < l && i < max) ; i++){
        ...
        validateLink:function(link){
            ...
            var XMLHttpTimeout = null,
                ...
                url = link.href,
                approvedCodes = linkAudit.getLocalStore("ignorePermissions").split(/,\s*/);
            ...
            xmlHttpRequest.onreadystatechange = function(oEvent){
                if (xmlHttpRequest.readyState === 4) {
                    ...
                    } else if (approvedCodes.indexOf(xmlHttpRequest.status.toString()) > -1) {
                        link["status"] = "success";
        ...
        checkForContentMatch:function(responseText){
            ...
            linkAudit.log("Searching responseText for content");
            for (var i = 0,str; i < linkAudit.contentStrings.length ; i++){
                str = linkAudit.contentStrings[i].trim();
                if (!str || str.length === 0){
                    continue;
                }
                if (linkAudit.contentRegExp.length <= i){
                    ...
                    linkAudit.contentRegExp.push( new RegExp( str , "i") );
                }
                linkAudit.log("validating link responseText "+ str);
                if (responseText.match( linkAudit.contentRegExp[i] )){
                    linkAudit.log('match found!!!');
                    return str;
                }
            }
            return "";
