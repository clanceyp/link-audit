	...
	content:{
		...
		createUI:function(){
			if ($('#CE-linkAuditDisplay').length === 0){
                ...
                var bg = linkAudit.ops.colour["pending"][0],
                    baseStyles = document.createTextNode("a.CE-linkAudit{background-color:"+bg+"!important;color:"+linkAudit.ops.colour["pending"][1]+";}");
        ...
        updateDisplay:function(linkArray){
            ...
            var total = linkArray.length,
                ...
                okay = _.filter(linkArray, function(link){return (link.httpStatusCode > 199 && link.httpStatusCode < 400 && link.status !=="content-match");}).length,
                ...
                cols = (linkAudit.ops.contentSearch) ? 4 : 3;

            if ($("#CE-linkAuditDisplay-status span").length == 0){
                for (var i = 0; i < cols; i++){
                    $("#CE-linkAuditDisplay-status").append("<span />");
                }
                $("#CE-linkAuditDisplay-status").addClass("CE-linkAudit-cols-"+cols);
            }
            if (cols === 4){
                $("#CE-linkAuditDisplay-status span").eq(0).text("Matched<br/>" + contentMatched);
                $("#CE-linkAuditDisplay-status span").eq(1).text("Failed<br/>" + failed);
                $("#CE-linkAuditDisplay-status span").eq(2).text("Okay<br/>" + okay);
                $("#CE-linkAuditDisplay-status span").eq(3).text("Queued<br/>" + queued);
            } else {
                $("#CE-linkAuditDisplay-status span").eq(0).text("Failed<br/>" + failed);
                $("#CE-linkAuditDisplay-status span").eq(1).text("Okay<br/>" + okay);
                $("#CE-linkAuditDisplay-status span").eq(2).text("Queued<br/>" + queued);
            }
