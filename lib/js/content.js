	...
	content:{
		...
		createUI:function(){
			if ($('#CE-linkAuditDisplay').length === 0){
                ...
                var baseStyles = document.createTextNode("a.CE-linkAudit{background-color:"+linkAudit.ops.colour["pending"][0]+";color:"+linkAudit.ops.colour["pending"][1]+";}");
        ...
        updateSingleLink:function(link){
            ...
            var style = document.createTextNode("a."+ link.id +" {background: "+ linkAudit.ops.colour[link.status][0] +" !important; color: "+ linkAudit.ops.colour[link.status][1] +" !important;}"),
            ...
            if (linkAudit.ops.showInfoInTitle){
                var status = (link.contentMatch === false) ? link.status + " " + link.httpStatusCode : "content found - "+ link.contentMatch ;
                $("a."+ link.id).attr("title","linkAudit status: "+ status);
            }
        },
