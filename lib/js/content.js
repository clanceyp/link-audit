    ...
    content:{
        ...
        reDraw:function(setup){
            ...
            if (linkAudit.ctxErrorHeight > 0){
                ctx.save();
                linkAudit.content.drawBox(ctx, 15, (ctxHEIGHT-linkAudit.ctxErrorHeight-10), linkAudit.ctxColumnWidth, (linkAudit.ctxErrorHeight), [2,2,0,0], true, false, "rgba(255, 0, 0, "+opacity+")");
                ctx.restore();
            }
            ...
            if (linkAudit.ctxColumnLength === 3){
                if (linkAudit.ctxMatchedHeight > 0){
                    ctx.save();
                    linkAudit.content.drawBox(ctx, (linkAudit.ctxColumnWidth*2) + (30), (ctxHEIGHT-linkAudit.ctxMatchedHeight-10), linkAudit.ctxColumnWidth, (linkAudit.ctxMatchedHeight), [2,2,0,0], true, false, "rgba(255,204,0,"+opacity+")");
                    ctx.restore();
                }
            }
        ...
        ctxInit:function(){
            ...
            /*
             ...
             // total
             ctx.save();
             linkAudit.content.drawBox(ctx, 10, 10, (ctxWIDTH-20), (linkAudit.ctxPendingHeight), 5, true, false, "rgba(80,80,80,0.6)");
             ctx.restore();

             // error
             ctx.save();
             linkAudit.content.drawBox(ctx, 20, (ctxHEIGHT-linkAudit.ctxErrorHeight-10), linkAudit.ctxColumnWidth, (linkAudit.ctxErrorHeight), [2,2,0,0], true, false, "rgba(255, 0, 0, 1)");
             ctx.restore();

             // okay
             ctx.save();
             linkAudit.content.drawBox(ctx, (linkAudit.ctxColumnWidth+11+20), (ctxHEIGHT-linkAudit.ctxSuccessHeight-10), linkAudit.ctxColumnWidth, (linkAudit.ctxSuccessHeight), [2,2,0,0], true, false,  "rgba(0,255,0,1)");
             ctx.restore();

             // conten-matched
             if (linkAudit.ctxColumnLength === 3){
             ctx.save();
             linkAudit.content.drawBox(ctx, (linkAudit.ctxColumnWidth+11)*2 + 20, (ctxHEIGHT-linkAudit.ctxMatchedHeight-10), linkAudit.ctxColumnWidth, (linkAudit.ctxMatchedHeight), [2,2,0,0], true, true, "rgba(255,204,0,1)");
             ctx.restore();//132
             }
             */
            }
