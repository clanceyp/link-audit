        ...
        showAudit:function(){
            if ($("#CE-linkAuditDisplay-details").hasClass('open')){
                ...
                var success = _.filter(linkAudit.linkArray, function(link){return (link.httpStatusCode > 199 && link.httpStatusCode < 400 && link.status !=="content-match");}).length,
                    ...
                    mMessage = (matched && matched.length) ? ", matched "+ matched.length : "",
                    html = '<tr><td colspan="2">Total: '+ linkAudit.linkArray.length +', okay '+ success +', failed '+failed +  mMessage +'</td></tr>',
                ...
                $("#CE-linkAuditDisplay-details").html(html).addClass('open');
            }
        },/**
