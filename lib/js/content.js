   ...
   "use strict";
    ...
    var ctxHEIGHT = window.ctxHEIGHT,
        ...
        linkAudit = {
            ...
            content:{
                ...
                createUI:function(){
                    ...
                    if ($('#CE-linkAuditDisplay').length === 0){
                        ...
                        $('body').append('<aside id="CE-linkAuditDisplay"><img id="CE-linkAuditDisplay-process" class="spin" src='+chrome.extension.getURL("lib/i/radiation-000.png")+' /><span class="close">&times;</span><span class="h2">Link Audit </span><div id="CE-linkAuditDisplay-status"><canvas id="CE-linkAuditDisplayCanvas" width="'+ctxWIDTH+'" height="'+ctxHEIGHT+'"></canvas></div><input type="button" value="sort"/><div id="CE-linkAuditDisplay-details-wrapper"><table id="CE-linkAuditDisplay-details"></table></div></aside>');
                        ...
                        $("#CE-linkAuditDisplay").on("click", "[data-download-csv]", function(){
                            linkAudit.util.downloadCSV();
                        });
                ...
                updateAuditTable:function(init){
                    ...
                    if (init){
                        $("#"+tableId).empty();
                        html = '<tr><th colspan="2" id="'+tableId+'-totals" data-download-csv title="Download as CSV">'+ totals +'</th></tr>';
                        for ( i = 0, l = sortedArray.length; i < l; i++){
                            var status;
                            link = sortedArray[i];
                            path =  link.path ;
                            status = link.status || "pending";
                            html+= '<tr id="CE-linkAudit-TR-'+ link.id +'" title="Status code: '+ link.httpStatusCode +'" class="CE-linkAudit-status-'+link.status+'"><td>'+ status +'</td><td><a target="_blank" href="'+ link.href +'" title="'+ link.href +'" data-http-status-code="'+link.httpStatusCode+'"> '+ path +'</a></td></tr>';
                        }
                        $("#"+tableId).html(html);
                        ...
                        for ( i = 0, l = linkAudit.linkArray.length; i< l; i++){
                            ...
                            if (loggedStatus !== "CE-linkAudit-status-"+link.status){
                                ...
                                $ref.attr("title", 'Status code: '+ link.httpStatusCode )
                                    .attr('class',"CE-linkAudit-status-"+link.status)
                                    .children("td:nth-child(1)").text( link.status )
                                    .attr('title', title );
                                $ref.find("td a").attr("data-http-status-code", link.httpStatusCode).text( path );
                            }
                        }
                    }
            ...
            util: {
                convertArrayToCSV: function (args) {
                ...
                downloadCSV: function(args) {
                    ...
                    args = args || {
                        data: linkAudit.linkArray
                    };

                    csv = linkAudit.util.convertArrayToCSV(args);

                    if (!csv) {
                        return;
                    }

                    filename = prompt("Please enter a file name") || 'linkAudit.'+ location.href.replace(/\W+/g, "-") +'.csv';

                    if (typeof(filename) !== "string"){
                        // it's probably null, ignore
                        return;
                    }
                    ...
                    link = linkAudit.util.link || document.createElement('a');
                    link.setAttribute('href', data);
                    link.setAttribute('download', filename);
                    link.dispatchEvent( event );

                    linkAudit.util.link = link;
                }
            }
        };
