    ...
    content:{
            ...
            if ($('#CE-linkAuditDisplay').length === 0){
				...
				if (linkAudit.linksToCheckLength){
					...
					var settingsMessage = (linkAudit.ops.whitelist.length || linkAudit.ops.blacklist.length) ? "blacklist and white list settings on the" : "",
						...
						message = '<div style="width:'+ctxWIDTH+'px"><p id="CE-linkAuditDisplay-nolink-message" id="#CE-linkAuditDisplay-nolink-message">No links found to validate, please check the extension '+settingsMessage+' <a href="'+settingsUrl+'" target="_blank">options page</a></p></div>';
		...
		updateAuditTable:function(init){
			...
			if (init){
				...
				for (var i = 0, title; i< linkAudit.linkArray.length; i++){
